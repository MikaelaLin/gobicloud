...
if($mysqli->query("UPDATE CHECKLIST SET USERID ='$userIDC', WORKSPACEID='$workspaceIDC',
    ...
    LASTUPDATE = CURRENT_TIMESTAMP, GEOLOCATION = '$geolocationC' ,TAG = '$tagC'
    WHERE CHECKLISTID=2")){
   $mysqli->commit();

   $result = $mysqli->query($sqleditC);
   //var_dump($resultC);
   ...
   /* write success, checklist information to array for JSON */
$checklistEdit = array('Result' => 'Success', 'CHECKLISTID' => $Cchecklist_id, 'USERID' => $Cuser_id, 'WORKSPACEID' => $Cworkspace_id, 
        ...
        'GEOLOCATION' => $Cgeo_location,'LASTUPDATE' => $Clast_update,'TAG' => $Ct_ag,);

/* echo JSON of the array for the phone */
echo 'This array has the information of the checklist: '.$checklistEdit;
